...
class QuadSerial(object):
    ...
    def __init__(self, fd="/dev/tty"):
        self._serial = None
        for serial_connection in QuadSerial.serials:
            self._serial = serial.Serial(serial_connection)
    ...
    # write to serial
    def write(buf):
        return os.write(self._serialfd, buf)

    def write_all(buf):
        while len(buf):
            buf = buf[self.write(buf):]
